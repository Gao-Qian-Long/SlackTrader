...
- 竞价图仅在详细图提供，使用精确的 `09:15:00–09:25:00` 独立坐标；`09:15` 左对齐、`09:20` 居中、`09:25` 右对齐，避免末端刻度越界。退出详细图或收回小窗时回到普通分时，避免隐藏按钮造成模式滞留。
- 只采集行情源返回且带当天源时间戳的真实竞价报价。记录竞价价和行情源累计量，再计算采样区间增量；不把源成交量宣称为未成交委托量，不根据盘口生成价格，也不把 `09:30` 普通分时冒充竞价历史。
- `09:15–09:25` 报价约 3 秒刷新，`09:25:00` 后不再加入新点；`09:25–09:30` 保留最终竞价图。下方用红绿灰量柱显示每个采样区间的源累计量增量，并标注行情源报告的累计股数。当天已采集点保存在本地，重启后仍可查看；公开分钟接口不含竞价历史时，明确标注“本次运行实时采集”，不补造启动前数据。
- 板块源没有经过验证的竞价口径时显示“板块不提供集合竞价”。测试应覆盖北京时间和周末边界、源时间过滤、累计量差分、详细图按钮、三个时间刻度以及返回普通分时。

## 日 K 成交量

- 日 K 使用独立的 `HistogramSeries` 展示接口返回的日成交量，量柱与蜡烛图共用日期轴和可见范围；成交量价格轴隐藏，底部保留约 22% 高度。
- 上涨或平盘日使用 `volumeUp`，下跌日使用 `volumeDown`；用户调整量柱颜色后应立即重绘现有日 K 量柱。
- 切股时同时清空旧 K 线和旧成交量，数据返回后一次性更新两个序列。缩放、拖动、双击复位应保持两个序列同步。

## 行情健康与本地诊断

- 报价、分时、日 K 按端点独立记录连续失败、最近成功时间、25% 新样本权重的延迟 EWMA、冷却时间和标准化错误类别；自动模式按健康状态排序，手动首选失败时仍允许回退。
- 冷却结束后允许恢复探测。现有报价、分时刷新间隔和请求缓存保持不变，健康调度不得额外增加自动轮询。
- 竞价报价超过 10 秒、连续交易报价超过 15 秒、分时超过 90 秒才显示延迟；盘前、午休和收盘不套用交易阈值。切源提示最多保留 10 秒。
- 用户主动“检测行情”时顺序检查当前标的经过验证的报价源，每个最多 5 秒，不修改当前图表。板块只检查自身真实来源。
- 本地诊断键为 `marketDiagnosticsV1`，最多 200 条、7 天、64 KB。只记录类别、来源、耗时、结果和错误码，不记录代码、名称、持仓、完整 URL 或响应正文，也不发送遥测。
