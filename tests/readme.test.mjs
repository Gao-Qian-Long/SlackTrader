...
test('README使用正式项目名称和当前版本',()=>{
  assert.match(readme,/^# SlackTrader（蓝牙助手）/);assert.ok(readme.includes(`> 当前版本：**v${version}**`));
});
...
test('README发布资产统一指向当前版本',()=>{
  for(const name of ['Setup-x64.exe','Portable-x64.exe','Source.zip'])assert.ok(readme.includes(`SlackTrader-v${version}-${name}`));
...
test('README说明当前已发布功能、详细图和量柱',()=>{
  for(const text of [`## v${version} 更新`,'安装版、便携版及源码均包含下列更新','详细图','返回小图','分钟成交量','780 × 440','关联板块对照','不是交易所逐笔成交'])assert.ok(readme.includes(text));
});
