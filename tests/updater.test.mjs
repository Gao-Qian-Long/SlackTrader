...
await test('发布清单版本、平台、签名内容及安装地址一致',()=>{
  const info=releaseInfo('0.5.0'),sig=Buffer.from('untrusted comment: test\nfixture').toString('base64');const m=updateManifest(info,sig,'notes','2026-09-03T00:00:00Z');assert.equal(m.platforms['windows-x86_64'].signature,sig);assert.equal(m.platforms['windows-x86_64'].url,'https://github.com/Gao-Qian-Long/SlackTrader/releases/download/v0.5.0/SlackTrader-v0.5.0-Setup-x64.exe');assert.equal(m.version,'0.5.0');assert.throws(()=>releaseInfo('0.5.0','v0.4'));assert.throws(()=>releaseInfo('0.5.0-beta'));assert.throws(()=>updateManifest(info,''));
});
